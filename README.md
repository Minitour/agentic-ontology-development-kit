...
flowchart TB
    ...
    Agent --> Reason["reasoning &amp; queries<br/><small>check_consistency · sparql_query</small>"]
    ...
    Editing -->|MCP| MCP["OWL-MCP Server<br/><small>axioms · reasoner · SPARQL · pitfalls</small>"]
    Reason -->|MCP| MCP
...
3. **Put existing OWL files under `projects/<project_dir>/`** (e.g. `projects/my-ontology/ontology/my-ontology.owl`) if you are editing an existing ontology. For a new one, create an empty OWL file under `projects/<project_dir>/ontology/` and describe what you want; the agent will build it up.
...
| Requirement             | Purpose                                                                                                                                          |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Node.js** (v18+)      | Runs capa and the owl-mcp server (via `npx`). [Install Node.js](https://nodejs.org/).                                                           |
...
- **New ontology (OBO/BFO style)**: Create an empty OWL file in `projects/<project_dir>/ontology/` and describe what you want. The agent will ask whether to use BFO and will add imports and alignment.
