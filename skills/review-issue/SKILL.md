...
Use **web fetch** (GitHub issue/API pages) or, if the repo is cloned under `projects/<slug>/`, you can combine with reading CONTRIBUTING/issue templates from disk. Do not name specific real-world ontologies as examples; use hypotheticals (e.g. "an OBO ontology", "a repo with NTR templates").
...
Use hypothetical examples only; do not name specific ontologies from a fixed list.
