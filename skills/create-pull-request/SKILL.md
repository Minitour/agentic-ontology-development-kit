...
5. **PR description**: Compose a PR body that fits the project's expectations (from **analyze-project** or CONTRIBUTING):
   ...
   - **Rationale**: Brief justification (e.g. design pattern, references, annotation impact) if the project uses it.
   - **Checklist**: If the project expects one (e.g. "pre-validation done", "term search done", "CI passes"), include a checklist and mark items or state that they are done.
6. **Open the PR**: If **GitHub CLI** (`gh`) is available and authenticated, run e.g. `gh pr create --title "..." --body "..."` from the clone root. Otherwise, output the PR title and body for the user to paste into the GitHub "Open pull request" form, and give the URL or steps to open the PR in the browser.
...
- **Checklist**: Items such as: plan reviewed, pre-validation done, term search done, edits done, automated validation (e.g. `make test`) passed. If the project has a PULL_REQUEST_TEMPLATE or CONTRIBUTING section on PRs, align with it.

Use hypothetical wording (e.g. "a repo that uses NTR templates"); do not name specific ontologies.
