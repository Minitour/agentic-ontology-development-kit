...
- **Do not modify OWL files**: Never directly edit OWL files by hand. Use the **ontology-editor** tools (OWL-MCP) for all axiom, prefix, and metadata changes. Use the owl-mcp reasoning and query tools (**check_consistency**, **sparql_query**) for consistency checking and CQ verification—not raw shell commands.
...
1. **Activate tools**: Read the **ontology-editor** skill (`skills/ontology-editor/SKILL.md`), then call `setup_tools(skills: ["ontology-editor"])` to activate the tools. If the MCP tools are unavailable (e.g. the editor hasn't reloaded the capa server after `capa install`), use the terminal CLI fallback instead: `capa sh owl <tool> --flag value` (kebab-case tool/flag names, repeat a flag for array values, absolute quoted paths, JSON output). See the ontology-editor skill for examples.
2. **Set ontology IRI**: Call `set_ontology_iri` with the **full IRI** (e.g. `"http://example.org/ontology/my-ontology/"`) — never a CURIE (e.g. `"ex:"`). Same for the version IRI. CURIEs in the `Ontology(...)` header produce files that downstream tools cannot parse.
...
- **Add `owl:imports`** for the upper ontology (if any) using the **canonical IRI** in angle brackets: `Import(<http://purl.obolibrary.org/obo/bfo.owl>)` for BFO, `Import(<https://w3id.org/sulo/>)` for SULO. Never use CURIEs in `Import(...)` axioms (e.g. `Import(obo:bfo.owl)` is invalid and will not parse).
...
2. **Consistency**: Call `check_consistency` (owl-mcp) on the OWL file to check for logical inconsistencies and unsatisfiable classes. It runs an OWL 2 EL reasoner (whelk, the ELK-equivalent) and returns `consistent`, `unsatisfiable_classes`, and the reasoner used. Report the actual tool output. Note: this reasons within OWL 2 EL — it will not detect inconsistencies that depend on non-EL DL constructs (cardinality restrictions, complex disjointness, etc.).
3. **Competency question verification (mandatory — every CQ must be verified)**: Delegate this to a **subagent** — launch a Task with the **cq-verification** skill (`skills/cq-verification/SKILL.md`). Provide the subagent with: the skill path, the project directory, the path to the approved proposal (for the CQ list), and the path to the ontology OWL file. The subagent handles the full procedure: create test data, write SPARQL queries for every CQ, and execute every query with `sparql_query` passing **both** the ontology and the test-data file as `owl_file_paths` (owl-mcp merges them in-memory). Queries must run against the merged schema + individuals — never against test data alone. Every CQ must appear in the results table. If any CQ fails due to an ontology gap, return to Step 6.
